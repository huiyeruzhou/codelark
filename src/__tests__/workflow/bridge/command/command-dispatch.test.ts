...
import { CONFIG_PATH, CONFIG_JSON_PATH, CODELARK_HOME, DEFAULT_WORKSPACE_ROOT } from '../../../../configuration/paths.js';
...
describe('command-dispatch', () => {
  ...
  it('renders /currnet as a green current-session card and saves name through the config form', async () => {
    ...
    assert.equal(sent.at(-1)?.richCard?.form?.selects?.some((select: any) => select.elementId === 'clk_sandbox'), false);
    assert.equal(getThreadTableMessageRecord(address, 'current')?.messageId, 'reply-2');
          ...
          event: {
            action: {
              form_value: {
                ...
                clk_model: 'test-model',
                clk_mode: 'normal',
                clk_provider: 'pty',
                clk_reasoning: 'medium',
              },
            },
          },
        },
  ...
  it('reads Claude Code JSONL for /t, /current, and /his on Claude runtime sessions', async () => {
    ...
    try {
      ...
      assert.match(sent.at(-1)?.text || '', /Claude permission.*plan/s);
      assert.match(sent.at(-1)?.text || '', /当前模型.*claude-sonnet-test/s);
