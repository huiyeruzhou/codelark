...
export type LocalSessionListRuntime = 'codex' | 'claude' | 'kimi' | 'cursor' | 'zcode';
  ...
  switch (rawCommand) {
    ...
    case '/t':
      return /^(ls|archive|rename|unbind)\b/i.test(args.trim())
        ...
        : !args
        ? '/threads'
        : /^(all|n\b|codex\b|claude\b|kimi\b|cursor\b|zcode\b|runtime\b)/i.test(args.trim())
          ? '/threads'
          : '/thread';
    case '/n':
      return '/new';
...
export function parseLocalSessionListArgs(args: string): { showAll: boolean; limit: number; runtime?: LocalSessionListRuntime } | null {
  ...
  if (parts[0] === 'codex' || parts[0] === 'claude' || parts[0] === 'kimi' || parts[0] === 'cursor' || parts[0] === 'zcode') {
