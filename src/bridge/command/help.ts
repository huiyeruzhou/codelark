...
export function buildHelpCommandResponse(): string {
  return [
    ...
    '- `//...` 向模型发送以 `/` 开头的文本',
    '- `/h` 帮助',
    '- `/provider` 或 `/p` 查看或切换当前 runtime 的 Provider（使用何种方式运行 agent）；Codex 可用 `sdk | pty | tmux`，Claude 可用 `tmux | pty | sdk` 且默认 `tmux`',
    '- `/stop` 停止当前任务',
    ...
    '- `/m` 查看 YOLO模式；可用 `normal | yolo`，YOLO模式允许 agent 无需审批绕过沙箱（`code` 会映射为 `normal`）',
