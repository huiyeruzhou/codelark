...
export function buildHelpCommandResponse(): string {
  return [
    ...
    '- `/his` 最近消息卡片（等同 `/his msg`，优先 Codex / Claude Code / Kimi Code 本地 session 文件，找不到再读 Bridge 缓存）',
    ...
    '- `//...` 向模型发送以 `/` 开头的文本',
    '- `/h` 帮助',
    '- `/provider` 或 `/p` 查看或切换当前 runtime 的 Provider（使用何种方式运行 agent）；Codex 可用 `sdk | pty | tmux`，Claude 可用 `tmux | pty | sdk` 且默认 `tmux`，Kimi 固定 `tmux`',
    '- `/stop` 停止当前任务',
    `- \`/t\` 默认显示当前 runtime 最近 ${DEFAULT_LOCAL_SESSION_LIST_LIMIT} 条本地会话；卡片是一张表，可用数量下拉切换 20/50/100，也可用 runtime 下拉切换 Codex/Claude/Kimi`,
    `- \`/t all\` 最多显示当前 runtime 最近 ${MAX_LOCAL_SESSION_LIST_LIMIT} 条本地会话`,
    `- \`/t n 50\` / \`/t n 100\` 调整当前 runtime 本地会话显示数量；也可用 \`/t codex n 100\`、\`/t claude n 100\` 或 \`/t kimi n 100\``,
    ...
    '',
    '**SessionRuntime 配置**',
    '- `/runtime codex|claude|kimi` 切换当前会话使用 Codex、Claude Code 或 Kimi Code；不会改变对应 runtime 已记住的 `/provider` 选择',
    ...
    '',
    '**GlobalRuntime 配置**',
    '- `/set` 打开 TOML 配置卡片；顶部下拉切换「通用配置 / [runtime.codex] Codex / [runtime.claude] Claude / [runtime.kimi] Kimi / Bridge / 通道配置（feishu-default）」',
    '- `/set runtime codex|claude|kimi` 修改默认 agent `runtime.agent`（兼容旧名 `defaultRuntime`）',
