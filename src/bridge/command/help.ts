...
export function buildHelpCommandResponse(): string {
  return [
    ...
    '- `/his` 最近消息卡片（等同 `/his msg`，优先 Codex / Claude Code session JSONL，找不到再读 Bridge 缓存）',
    ...
    `- \`/t n 50\` / \`/t n 100\` 调整当前 runtime 本地会话显示数量；也可用 \`/t codex n 100\` 或 \`/t claude n 100\``,
    ...
    '',
    '**GlobalRuntime 配置**',
    '- `/set` 打开 TOML 配置卡片；顶部下拉切换 `[runtime]`、`[runtime.codex]`、`[runtime.claude]`、`[bridge]`、`[[channels]]`',
    '- `/set runtime codex|claude` 修改 `runtime.agent`（兼容旧名 `defaultRuntime`）',
    '- `/set defaultModel|defaultMode|codexSandboxMode|codexNetworkAccess|codexReasoningEffort ...` 修改 `[runtime.codex]`',
    '- `/set claudeProvider|claudeExecutable|claudeDefaultModel|claudePermissionMode|claudeIdleTimeoutMinutes ...` 修改 `[runtime.claude]`',
    '',
    '**GlobalBridge 配置**',
    '- `/set defaultWorkspaceRoot|uiAllowLan|uiAccessToken ...` 修改 `[bridge]`；`/set historyMessageLimit ...` 修改默认 Feishu `[[channels]]`',
