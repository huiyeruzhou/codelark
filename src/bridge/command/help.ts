...
export function buildHelpCommandResponse(): string {
  return [
    ...
    '- `//...` 向模型发送以 `/` 开头的文本',
    '- `/h` 帮助',
    '- `/provider` 或 `/p` 查看或切换当前 runtime 的 Provider transport；Codex 可用 `sdk | pty | tmux`，Claude 可用 `pty | tmux | sdk`',
    '- `/stop` 停止当前任务',
    ...
    '',
    '**GlobalBridge 配置**',
    '- `/set defaultWorkspaceRoot|historyMessageLimit|uiAllowLan|uiAccessToken ...` 修改 Bridge 自身配置',
