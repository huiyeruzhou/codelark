...
Linux、macOS 和 Windows job 都先安装固定版本的 Codex/Claude/Kimi CLI，再运行上述 real-runtime gate。gate 从安装 smoke 导出的绝对 executable 路径执行 availability probe，不能再用裸命令做第二套 PATH 判断；TUI 随后仍通过各 runtime 的正式 resolver 启动。Codex shard 串行包含多个各自带 timeout 的真实测试，scheduler 的文件级 budget 必须覆盖整组，而不能误用单个 test 的 timeout；fresh 临时 workspace 可能出现 trust prompt，测试必须同时等待 turn 完成或 prompt 出现，慢 runner 上不能用短暂“未看到”当作“无需确认”。Kimi 当前固定为 npm latest 对应的具体版本，而不是浮动 dist-tag；gate 会关闭 Kimi 自更新，保证该版本在完整 lifecycle 内不可变，否则 npm global 后台替换 executable 会与 kill/resume 竞态。Kimi 0.34.0 默认 v2 会延迟到首条消息后才创建 session，CodeLark 启动时使用其官方 legacy compatibility flag 保持可持久化的 eager session identity。Kimi workspace trust 确认每个对话只能注入一次 Enter；慢 runner 上画面延迟重绘不得导致第二个 Enter 落入刚出现的 editor。
