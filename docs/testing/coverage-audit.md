...
| 自动化输入 | `/every` 创建/列表/取消、表单新建和 every card 回调；`/then` 创建/列表/修改/取消、卡片新建/修改/取消、长 prompt 折叠和一次性后续发送。 | `bridge-command-e2e.test.ts`、`command-dispatch.test.ts` | `command-state` provider 矩阵通过真实飞书消息覆盖 `/every` 创建/列表/取消，并断言 created/list/deleted 文本；`/then` 当前已有本地 workflow 证据，真实飞书 command-state 后续应补卡片 submit/callback 证据 |
