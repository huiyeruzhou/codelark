...
| --- | --- | --- | --- |
| Runtime 路由 | 普通 IM 消息进入选定 runtime；Claude 默认走 tmux，slash 命令仍在 bridge 内联处理。 | `bridge-adapter-runtime.test.ts`、`interactive-turn-runner.test.ts`、`session-runtime.test.ts`；`bridge-command-e2e.test.ts` 覆盖显式 `/runtime kimi` + `/p tmux` 后的普通飞书消息进入 Kimi tmux、idle 不发送 `Ctrl-S`、绑定 Kimi session/cwd，并以 `mirror:` 返回 `**kimi:**` final；active steer 由状态机与真实 Kimi gate 覆盖。 | `message-only`、`runtime-message` 已有 Codex/Claude canonical 报告；`message-only::kimi-tmux` 和 `runtime-message::kimi-tmux` 也已有真实飞书 canonical 报告，证明隔离 test app、隔离 bridge、Kimi auth/config 隔离复制、真实飞书发送/读取、Kimi `wire.jsonl`、provider output path、runtime identity 绑定和 mirror 去重链路。有稳定 marker 的后续场景继续通过 `runtime_prompt_final_transcript_marker` 从最终飞书 transcript 命名验证 runtime prompt 最终回复 |
...
| Runtime 配置 | `/model`、`/mode`、`/sandbox`、`/network`、`/reasoning`、全局 `/set`。 | `command-dispatch.test.ts`、`bridge-command-e2e.test.ts`、`config.test.ts`；覆盖 Codex `max/ultra`、Claude 独立 effort、Kimi `default/on/off` Thinking、Cursor 参数化模型 effort，以及 ZCode model/mode 与固定 tmux，且各 runtime 不串写配置。 | `command-state` 覆盖 runtime 设置并带命令级飞书文本断言；Kimi 仍应证明固定模式、不支持 Bridge 沙箱/网络，同时 `/reasoning` 展示或写入 Thinking 开关；ZCode 证明 `/reasoning` 不映射且 `//` 原生命令透传；`session-management` 覆盖 `/set`。 |
...
| Kimi Code 集成 | Kimi tmux provider、fresh CLI 随机 session id、输入前/后两种 `wire.jsonl` 创建时序、`think` 状态区展示，以及仅对 active turn 执行的 `Ctrl-S` steer。 | `kimi-tmux-provider.test.ts`、真实 Kimi executable E2E、`kimi-tmux-provider-local-process.e2e.test.ts`、`kimi-session-index.test.ts`、`mirror-runtime.test.ts`、`command-dispatch.test.ts`、Operator UI/session registry 测试；workflow 覆盖 lazy wire 与 idle 无 steer，真实 executable + fake proxy 覆盖 fresh、running steer、复用、tmux 丢失恢复和错误终态。 | real-Feishu `message-only::kimi-tmux` 和 `runtime-message::kimi-tmux` 已用真实 Kimi + 隔离 `KIMI_CODE_HOME` 证明 `/runtime kimi`、`/p tmux`、真实 IM 发送/读取、Kimi `wire.jsonl`、provider output path 和 mirror final 去重；0.2.0 发布还要求当前 revision 重跑一次用户可见 Kimi 热点故事。 |
