...
| Runtime 配置 | `/model`、`/mode`、`/sandbox`、`/network`、`/reasoning`、全局 `/set`。 | `command-dispatch.test.ts`、`bridge-command-e2e.test.ts`、`config.test.ts`；覆盖 Codex `max/ultra`、Claude 独立 effort、Kimi `default/on/off` Thinking 开关和 Cursor 参数化模型 effort，且各 runtime 不串写配置。 | `command-state` 覆盖 runtime 设置并带命令级飞书文本断言；Kimi 仍应证明固定模式、不支持 Bridge 沙箱/网络，同时 `/reasoning` 展示或写入 Thinking 开关；`session-management` 覆盖 `/set`。 |
