...
| 定时输入 | `/every` 创建/列表/取消、表单新建和 every card 回调。 | `bridge-command-e2e.test.ts`、`command-dispatch.test.ts` | `command-state` provider 矩阵通过真实飞书消息覆盖创建/列表/取消，并断言 created/list/deleted 文本 |
