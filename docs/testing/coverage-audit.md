...
| Provider 选择 | `/provider` 和 `/p` 切换 Codex `sdk|pty|tmux` 与 Claude `pty|tmux|sdk`。 | `command-dispatch.test.ts`、`bridge-command-e2e.test.ts`、`codex-routing-provider.test.ts` | `runtime-message`、`command-state`、`session-management` provider 矩阵 |
