...
真实飞书 E2E 不应逐条镜像单元测试，而应通过基础对话 suite 和功能 suite 覆盖高风险、客户端可见的工作流，并在一次运行中收集多项相关断言。诊断、终端和会话控制命令 suite 应按 runtime（`codex` 和 `claude`）检查，而不是按完整 provider 矩阵检查；只有 tmux 专属命令需要额外的 `codex-tmux` provider 路径。
...
| --- | --- | --- | --- |
| Runtime 路由 | 普通 IM 消息进入选定 runtime；slash 命令仍在 bridge 内联处理。 | `bridge-adapter-runtime.test.ts`、`interactive-turn-runner.test.ts`、`session-runtime.test.ts` | `message-only`、`runtime-message` 覆盖 codex-sdk/codex-pty/codex-tmux/claude-pty/claude-sdk |
| Provider 选择 | `/provider` 和 `/p` 切换 Codex `sdk|pty|tmux` 与 Claude `pty|sdk`。 | `command-dispatch.test.ts`、`bridge-command-e2e.test.ts`、`codex-routing-provider.test.ts` | `runtime-message`、`command-state`、`session-management` provider 矩阵 |
...
| Claude Code 集成 | Claude pty onboarding/trust setup、CCR 激活、Claude JSONL 身份、Claude SDK query 路径。 | `claude-pty-provider.test.ts`、`claude-sdk-provider.test.ts`、`real-claude-pty-provider.e2e.test.ts`、`claude-session-jsonl.test.ts` | Claude pty 报告使用真实 `ccr code` + fake backend；Claude SDK 报告使用 CCR-backed SDK env + fake backend |
