...
| Kimi Code 集成 | Kimi tmux provider、resume hint session id、`wire.jsonl` mirror、`think` 状态区展示和自动 `Ctrl-S` steer。 | `kimi-tmux-provider.test.ts`、`kimi-tmux-provider-local-process.e2e.test.ts`、`kimi-session-index.test.ts`、`mirror-runtime.test.ts`、`command-dispatch.test.ts`、Operator UI/session registry 测试；`bridge-command-e2e.test.ts` 现在覆盖 `/t` 绑定 Kimi `wire.jsonl` 后 `/his` 默认、`msg`、`raw`、`json`、`file` 读取同一 Kimi wire 文件且排除 user/think/Bridge 缓存；`bridge-manager.test.ts` 覆盖 Kimi thinking 只进入 mirror stream 状态区、被状态字数截断且不进入 stream 正文或最终 turn text；`feishu-adapter-card-e2e.test.ts` 覆盖 Kimi mirror markdown final 经过真实 `FeishuAdapter.send` 生成 interactive markdown card，且 final 卡片不泄漏 `think`/「当前思考」状态内容。 | local-process 覆盖 fake Kimi CLI + 真实 tmux 的 deterministic smoke；real-Feishu `message-only::kimi-tmux` 和 `runtime-message::kimi-tmux` 已用真实 Kimi + 隔离 `KIMI_CODE_HOME` 证明 `/runtime kimi`、`/p tmux`、真实 IM 发送/读取、Kimi `wire.jsonl`、provider output path 和 mirror final 去重。`basic-dialogue-suite --scripted-basic-dialogue` 已有 dry-run gate 和 proxy/Kimi 断言，但当前真实 run 诊断失败在进入 Kimi 前的 Codex SDK queued followup 收尾；该 suite 的 Kimi resume hint、Ctrl-S、thinking/status 和 history transcript gate 仍需在修复前置问题后跑出 canonical 报告。 |
...
| 附件和 artifact | 出站 artifact、问题卡片/表单、云文档评论交付。 | `outbound-artifacts.test.ts`、`delivery-pipeline.test.ts`、`command-dispatch.test.ts`；`bridge-command-e2e.test.ts` 覆盖 Kimi `/t` 绑定后 mirror `<clk-ask>` 输出被清理成 Feishu question form，带 `clk_choice`、`clk_input`、`clk-agent-question` callback，且 Kimi tmux 自动 `Ctrl-S`。`feishu-adapter-card-e2e.test.ts` 还覆盖同一个 Kimi mirror final 同时包含 Markdown 表格/代码块和 `<clk-ask>` 时，最终 Markdown card 不泄漏 ask/thinking，question form 作为独立 CardKit 表单发送。 | `command-state` provider 矩阵计划发送小文件 `/file <fixture>` 并要求 Feishu `file` 回复和 `file_key`，同时发送超过 20 MB 的 sparse fixture 并要求 Feishu `interactive` 大文件确认卡、文件名和 `clk-command` callback 前缀；`command_state_file_and_large_file_transcript` 会从最终飞书 transcript 命名 gate 这些 file/card 回复，覆盖 Codex/Claude/Kimi runtime 下的小文件发送命令与大文件确认入口。出站 agent artifact、云文档评论附件和真实确认 submit callback 事件仍未由 canonical 真实飞书矩阵覆盖 |
...
- 非 final 命令回复中的最终文本断言：`command-state` 现在检查 status/settings/runtime/provider/current-state/every 命令的语义文本，并通过 `command_state_runtime_settings_transcript` 从最终飞书 transcript 命名验证这些回复；Kimi tmux 路径必须证明 Kimi Code 固定模式、不支持 Bridge 沙箱/网络/思考设置，以及 `/every` 创建/列表/取消的 runtime 绑定文本。`command-state` 还会对 `/file <fixture>` 检查 Feishu file message 与 `file_key`；`session-management` harness 检查 help/config/new/clear/cd/shell/current/check 命令文本，其中 `/shell --sandbox read-only printf <marker>` 要求读回 `/shell 执行完成`、stdout marker、`Codex sandbox`、`read-only` 和退出码 `0`，最终报告还会通过 `session_management_runtime_identity_transcript` 从飞书 transcript 命名验证 `/current`、`/check` 和 `/t archive` 的 runtime identity/归档回复。codex-pty/codex-tmux 已有当前语义通过证据。旧 `session-management` 报告中的 codex-sdk/claude-pty/claude-sdk 仍只证明遗留 `reply_to` 证据，必须在新语义 gate 下重跑。一次旧的非隔离 `session-management::codex-sdk` 诊断证明命令链可以到达飞书和 `/his 5`，但因为当时 bridge provider 状态不受 harness 控制，canonical provider-output gate 失败。旧 `session-management::codex-tmux` 诊断失败已被 `session-codex-tmux-semantic-live-0550.json` 取代，该报告证明 mirror-stream final observation、`/his 5` marker、provider path 和双重清理。剩余命令密集型场景还需要对 permission 和 card callback 路径做同等处理。
...
- 飞书渲染：Markdown 表格和 fenced code block 现在已有 `markdown-rendering::codex-pty` 与 `markdown-rendering::codex-tmux` canonical 真实飞书通过证据；当前真实飞书 mirror/card transcript 会把 `ts` fence 规范化为 `plain_text`，因此 `markdown_rendering_transcript_structure` gate 检查代码 fence 保留和规范化后的语言，并适用于 Kimi tmux 等 planned runtime/provider 路径。Kimi tmux 现在有 mock FeishuAdapter E2E 证明 `wire.jsonl` mirror final 中的表格和 fenced code 会生成 `interactive` markdown card，且 `think`/「当前思考」不进入 final 卡片正文；同一测试族也覆盖 Markdown 正文与 `<clk-ask>` 同时出现时，ask block 从 final markdown 中剥离并作为独立 question form 发送。但 `markdown-rendering::kimi-tmux` 仍缺真实飞书发送与 transcript 读取后的 canonical 报告。剩余 Markdown provider canonical 报告、嵌套代码块、工具详情卡片、长流式更新和 mirror 控制通知仍主要依赖本地测试。
- 权限和恢复体验：缺少读消息权限、缺少删除权限、缺少事件订阅、bot 不在群里、重复 App 实例争抢。
- 服务/UI 操作：setup UI 和 service manager 行为主要依赖本地测试，因为它们不自然地由飞书聊天消息驱动。

## 下一批测试切片

`runtime-message::kimi-tmux` 已有真实飞书 canonical 报告，并证明 Kimi runtime prompt 最终文本可从最终飞书 transcript 读回；`command-state`、`session-management`、`history-suite`、`markdown-rendering` 等有稳定 final marker 的 runtime 场景也使用同一 gate，但仍需各自的真实飞书 canonical 报告。`history-suite::codex-tmux` 已有 canonical 证据；`history-suite::kimi-tmux` 已进入同一矩阵但仍需真实飞书 canonical 报告。下一批高价值切片仍是 `basic-dialogue-suite`，作为主要长流程集成测试。它的 harness metadata 和 dry-run plan 已描述一条 cross-provider 对话，本地 session simulator 已覆盖 `codex-sdk -> claude-sdk -> kimi-tmux -> codex-tmux -> claude-pty -> codex-pty` 上可控的 preload/tool/context/goal/mirror-suppression/Kimi steer 检查点；`--scripted-basic-dialogue` 已把 Codex Responses、Claude/CCR 和 Kimi executable 都隔离为确定性测试入口，其中 Kimi 仍经过真实 tmux/resume-hint/`Ctrl-S`/`wire.jsonl` mirror，且成功 gate 必须证明 Kimi think 映射出的「当前思考」只出现在非 final 流式状态区 checkpoint、不会泄漏到 completed final card，从 scripted Kimi 日志证明两次 Ctrl-C 产生 resume hint、`kimi -r <session>` 恢复和 Ctrl-S steer，从聊天绑定里的 Kimi runtime slot 对应 `wire.jsonl` 读回本轮 think、marker 文本和 `step.end`，并证明历史 transcript 读取路径能读回 marker 且不会把 thinking/status 内容写入历史正文。当前真实诊断报告 `work/real-feishu/real-feishu-basic-dialogue-scripted-kimi.failure.json` 卡在 Kimi 阶段之前：Codex SDK queued followup 已显示 `FOLLOWUP_ACK`，但 turn 未释放，导致 `/runtime claude` 排队超时；接受 basic-dialogue canonical 前必须先修复该前置收尾问题。命令密集型 suite 应继续按 runtime 压缩。历史 suite 应保持以下 gate 作为回归准则：
