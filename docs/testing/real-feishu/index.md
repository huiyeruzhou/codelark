...
- `command_state_runtime_settings_transcript`：`command-state` 必须从最终飞书 transcript 中读回 `/status`、`/require-at off`、`/runtime`、`/p`、`/current`、`/model`、`/mode`、`/provider`、`/sandbox`、`/network`、`/reasoning` 和 `/every` 创建/列表/取消回复；`/current` 还必须包含三项通用 tmux 会话配置和“跟随上层配置”选项。Kimi tmux 路径必须证明 Kimi Code 的固定模式和不支持 Bridge 沙箱/网络/思考设置文案。
