...
当前流式卡片 header 会使用 `bridge_id:<短 ID>` 标识 Bridge 会话来源；`/t` 会话表也显示短 `bridge_id`，卡片首列会标出当前激活的对话和其他人激活的对话。工具调用沿用“历史记录”外层容器，每个工具在其中直接对应一个折叠面板；展开后直接显示受字符数和行数双上限约束的真实参数、输出或 patch，不再为长输出增加内层折叠。工具面板按运行状态显示边框色。飞书面板边框色会先按 CardKit 支持的枚举和 rgba 格式过滤，避免无效颜色导致整张卡片被拒绝。
