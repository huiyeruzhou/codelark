...
- 色彩：暖灰蓝页面、白色内容、深蓝灰文字和蓝紫主操作；允许在页面背景、品牌和选中态使用低强度蓝紫渐变，runtime 身份色用于卡片顶部标记。绿色/蓝色/琥珀/红色只表达运行、排队、待处理和错误，不能同时承担装饰语义；
- 字体：系统 sans 负责界面，`ui-monospace` 只用于 ID、路径、命令和日志；使用固定字号阶梯，不使用流式大标题；
- 形状：8–16px 圆角、明确的 surface 层级和柔和阴影；不用全页 glass blur、渐变文字和夸张宽阴影，但不能把“去装饰”误解成灰框调试页；
...
- route/application：配置校验、session materialize/归档、具体 chat binding、日志过滤和服务状态；
