...
兼容路径：若模型已经在 assistant 正文中输出绝对本地路径形式的 Markdown 图片（例如 `![二维码](/tmp/qr.png)`），Bridge 会将它提取为图片附件，走与 `<clk-send>` 相同的飞书上传与回复链路，并从卡片正文移除该本地路径；这不是把本地路径直接当作 CardKit image key。上传失败会在终态重试，最终失败时向原聊天回复文件名和错误原因。新提示词仍应优先使用 `<clk-send>`，以便明确表达发送意图。
