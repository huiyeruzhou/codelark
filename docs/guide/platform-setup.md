...
> **setup 用户授权：** `codelark setup` 只以 `~/.codelark/config.toml` 中的当前 App 为准；旧版 `config.json` / `config.env` 仅作为首次迁移输入。已有 App 配置时直接加载；没有 App 配置时通过开放平台扫码创建，`App ID` / `App Secret` 来自扫码返回结果。用户授权直接作用于全局 lark-cli 环境（`~/.lark-cli`）：setup 只做授权检查与 `auth login`，不修改全局环境已绑定的 App。生产 bridge 的云文档建群路径走机器人 OpenAPI，不依赖用户身份 CLI。控制台里的 tenant 权限和事件订阅仍需要按本页配置、发布并审批。
